    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 w-full">
      {stats.map((stat) => {
        ...
        return (
          <div 
            key={stat.category} 
            className={`p-4 rounded-xl border flex flex-col items-center justify-center text-center transition-all hover:scale-105 duration-300 ${getCategoryColor(stat.category)}`}
          >
            <div className="w-full flex justify-between items-center mb-2">
              <span className="font-bold text-lg">{stat.category}</span>
